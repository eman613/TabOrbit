...
const DEFAULT_CONFIG: &str = include_str!("../taborbit.ini");
const CONFIG_FILE_NAME: &str = "taborbit.ini";
const LEGACY_CONFIG_FILE_NAME: &str = "window-switcher.ini";
...
pub fn load_config() -> Result<Config> {
    let filepath = resolve_config_path()?;
    let opt = ParseOption {
...
pub(crate) fn edit_config_file() -> Result<bool> {
    ...
    if !filepath.exists() {
        let legacy_filepath = get_legacy_config_path()?;
        if legacy_filepath.exists() {
            fs::copy(&legacy_filepath, &filepath).map_err(|err| {
                anyhow!(
                    "Failed to migrate config file '{}' to '{}', {err}",
                    legacy_filepath.display(),
                    filepath.display()
                )
            })?;
        } else {
...
fn get_legacy_config_path() -> Result<PathBuf> {
    let folder = get_exe_folder()?;
    Ok(folder.join(LEGACY_CONFIG_FILE_NAME))
}
