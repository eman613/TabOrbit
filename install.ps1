$command = "taborbit"
$repo = if ($env:TABORBIT_REPOSITORY) { $env:TABORBIT_REPOSITORY } else { "eman613/TabOrbit" }
...
if ($Host.UI.RawUI.KeyAvailable) {
    $ans = Read-Host -Prompt "Run taborbit.exe? (y/n)"
    if ($ans -eq "y") {
        & $outfile
        Write-Host "Run taborbit.exe successful!"
        exit
    }
}
